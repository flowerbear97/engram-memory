...
|---|---|---|---|---|
| 安装方式 | `pip install` | `pip install` | Docker + Server | pip + Neo4j |
...
```bash
pip install engram              # 核心
pip install engram[openai]      # + OpenAI 嵌入
pip install engram[local]       # + sentence-transformers
pip install engram[mcp]         # + MCP 服务器
pip install engram[api]         # + REST API (FastAPI)
pip install engram[langchain]   # + LangChain 适配器
pip install engram[llamaindex]  # + LlamaIndex 适配器
pip install engram[telemetry]   # + OpenTelemetry
pip install engram[all]         # 全部
```
