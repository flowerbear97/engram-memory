"""Engram Benchmark — inspired by LongMemEval (ICLR 2025).

Evaluates Engram's memory capabilities across 5 dimensions:
  1. Information Extraction — retrieve specific facts from stored memories
  2. Multi-Session Reasoning — cross-session retrieval and filtering
  3. Temporal Reasoning — recency-aware retrieval and time-based ordering
  4. Knowledge Updates — handle contradicting/updated information
...
class BenchmarkReport:
    ...
    def print_report(self) -> None:
        ...
        print("\n" + "=" * 72)
        print("  ENGRAM BENCHMARK REPORT — LongMemEval Dimensions")
        print("=" * 72)
...
async def test_cross_namespace_isolation(mem: AgentMemory) -> TestResult:
    ...
    await mem.aremember("Use PostgreSQL for production", user_id="dev", namespace="project_alpha")
    await mem.aremember("Use MongoDB for analytics", user_id="dev", namespace="project_beta")
...
async def test_bulk_insert_performance(mem: AgentMemory) -> TestResult:
    ...
    for i in range(500):
        await perf_mem.aremember(
            f"Performance test memory number {i} with some content about topic {i % 50}",
            user_id="perf_user",
            type="fact",
        )
    ...
    throughput = 500 / (insert_time / 1000)
    return TestResult(
        dimension="Performance",
        test_name=f"Bulk insert 500 memories ({throughput:.0f} ops/s)",
        passed=stats.total_memories == 500,
        detail=f"{insert_time:.0f}ms total, {insert_time/500:.1f}ms/op",
    )
...
async def test_recall_latency(mem: AgentMemory) -> TestResult:
    ...
    for i in range(200):
        await perf_mem.aremember(
            f"Memory about topic {i}: details about item {i} in category {i % 20}",
            user_id="perf_user",
            type="fact",
        )
...
def main() -> None:
    print("\n🧠 Running Engram Benchmark (LongMemEval Dimensions)...\n")
