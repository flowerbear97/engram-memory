...
from neuragram.lifecycle.decay import DecayManager, DecayResult
from neuragram.lifecycle.forgetting import ForgettingManager
from neuragram.processing.embeddings import (
    BaseEmbeddingProvider,
    create_embedding_provider,
)
from neuragram.processing.llm import BaseLLMProvider, create_llm_provider
...
class AgentMemory:
    ...
    def _enforce(
        ...
        """Enforce access control for the current actor.

        When ``AccessPolicy.enabled`` is False (the default), this is
        a no-op.  When enabled, raises ``AccessDeniedError`` if the
        actor lacks the required permission level.
        """
        if not self._access_policy.enabled:
            return
        self._access_policy.enforce(
    ...
    async def aremember(
        ...
        self._enforce(AccessLevel.WRITE, "remember", namespace=namespace, user_id=user_id)
    ...
    async def arecall(
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.READ, "recall", namespace=namespace, user_id=user_id)
    ...
    async def aget(self, memory_id: str) -> Memory | None:
        ...
        await self._ensure_initialized()
        self._enforce(AccessLevel.READ, "get")
    ...
    async def aupdate(
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.WRITE, "update")
    ...
    async def aforget(
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.ADMIN, "forget", user_id=user_id)
    ...
    async def ahistory(self, memory_id: str) -> list[MemoryVersion]:
        ...
        await self._ensure_initialized()
        self._enforce(AccessLevel.READ, "history")
    ...
    async def adecay(self, max_age_days: int | None = None) -> dict[str, int]:
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.ADMIN, "decay")
    ...
    async def alist(
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.READ, "list", namespace=namespace, user_id=user_id)
    ...
    async def astats(self) -> StoreStats:
        ...
        await self._ensure_initialized()
        self._enforce(AccessLevel.READ, "stats")
    ...
    async def asmart_remember(
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.WRITE, "smart_remember", namespace=namespace, user_id=user_id)
    ...
    async def aprocess_conversation(
        ...
        await self._ensure_initialized()
        self._enforce(
            AccessLevel.WRITE, "process_conversation",
    ...
    async def aconsolidate(
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.WRITE, "consolidate", namespace=namespace, user_id=user_id)
    ...
    async def aexplain(
        ...
        """
        await self._ensure_initialized()
        self._enforce(AccessLevel.READ, "explain", namespace=namespace, user_id=user_id)
