...
logger = logging.getLogger("neuragram.store.sqlite")
...
# Each function receives an aiosqlite.Connection and migrates FROM
# (version-1) TO (version).  Only add new entries here for future
# schema changes; never modify existing ones.
_MIGRATIONS: dict[int, Callable[[aiosqlite.Connection], Awaitable[None]]] = {
    # Version 1 is the baseline — created by _SCHEMA_SQL + _schema_meta.
...
class SQLiteMemoryStore(BaseMemoryStore):
    ...
    Thread-/coroutine-safe: all write operations are serialized via an
    internal ``asyncio.Lock`` to prevent "database is locked" errors
    when used from concurrent coroutines (e.g. REST API handlers).

    Args:
        db_path: Path to the SQLite database file, or ":memory:" for in-memory.
        dimension: Dimensionality of embedding vectors.
        cache_size: Max entries in the LRU cache for ``get()``.
    """
    ...
    def __init__(
        ...
        # Write serialization lock — lazily created in initialize() to
        # avoid requiring an event loop at construction time (Python 3.9).
    ...
    async def initialize(self) -> None:
        ...
        # Initialize FTS5 virtual table (graceful degradation)
        ...
        try:
            ...
            # FTS5 module not compiled into this SQLite build
            pass
        ...
        for target_version in range(current_version + 1, _CURRENT_SCHEMA_VERSION + 1):
            migration_fn = _MIGRATIONS.get(target_version)
            if migration_fn is not None:
                logger.info(
                    "Applying schema migration v%d → v%d",
                    target_version - 1, target_version,
                )
                await migration_fn(self._db)
            await self._db.execute(
    ...
    async def _insert_one(self, db: aiosqlite.Connection, memory: Memory) -> str:
        """Insert a single memory into all indexes (no commit, no lock).

        This is the internal workhorse shared by ``insert()`` and
    ...
    async def insert(self, memory: Memory) -> str:
        ...
        async with self._write_lock:
            mid = await self._insert_one(db, memory)
            await db.commit()
            return mid

    async def batch_insert(self, memories: list[Memory]) -> list[str]:
        """Insert multiple memories in a single atomic transaction.

        If any insert fails, the entire batch is rolled back and the
        error is raised.  This is both faster (one COMMIT) and safer
        (all-or-nothing) than looping over ``insert()``.
        """
        if not memories:
            return []
    ...
    async def update(self, memory_id: str, **fields: object) -> Memory:
        db = self._ensure_open()
        async with self._write_lock:
            return await self._update_locked(db, memory_id, **fields)

    async def _update_locked(
        self, db: aiosqlite.Connection, memory_id: str, **fields: object
    ) -> Memory:
        ...
        for key, value in fields.items():
            if key == "content" and value is not None:
                ...
                set_params.append(value)
            elif key == "last_accessed_at" and value is not None:
                set_parts.append("last_accessed_at = ?")
                set_params.append(
                    _dt_to_str(value) if isinstance(value, datetime) else value
                )
            elif key == "status" and value is not None:
                set_parts.append("status = ?")
                set_params.append(
                    value.value if isinstance(value, MemoryStatus) else str(value)
                )
        ...
        if content_changed and self._fts_available:
            await db.execute(
    ...
    async def delete(self, memory_id: str, hard: bool = False) -> bool:
        db = self._ensure_open()
        async with self._write_lock:
            return await self._delete_locked(db, memory_id, hard=hard)

    async def _delete_locked(
        self, db: aiosqlite.Connection, memory_id: str, hard: bool = False
    ) -> bool:
        if hard:
    ...
    async def delete_by_filter(
        ...
        db = self._ensure_open()
        async with self._write_lock:
            return await self._delete_by_filter_locked(db, filters, hard=hard)

    async def _delete_by_filter_locked(
        self,
    ...
    async def stats(self) -> StoreStats:
        ...
        result = StoreStats(
            ...
            expired_memories=row[3] or 0,
            deleted_memories=row[4] or 0,
            total_users=row[5] or 0,
            total_namespaces=row[6] or 0,
