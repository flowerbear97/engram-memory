...
Metrics:
    - neuragram.memories.total: Gauge of total active memories
    - neuragram.operations.count: Counter of operations by type
    - neuragram.operations.duration: Histogram of operation latencies
    - neuragram.recall.results: Histogram of result counts per recall

Usage::

    # Auto-instruments when OpenTelemetry is available
    from neuragram.core.telemetry import get_tracer, get_meter
...
def get_tracer(name: str = "neuragram") -> Any:
...
def get_meter(name: str = "neuragram") -> Any:
...
_tracer = get_tracer("neuragram")
_meter = get_meter("neuragram")
